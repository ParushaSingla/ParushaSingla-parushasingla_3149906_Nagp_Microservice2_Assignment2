...
public class AggregatorServiceController {
	...
	public AggregatedResult userDetails(@PathVariable("id") int id) {
		ResponseEntity<User> userDetails = restTemplate.getForEntity(userUrl+"/user/{id}",
				User.class, id);
		ResponseEntity<OrderList> ordersOfUser = restTemplate.getForEntity(orderUrl+"/orders/{id}",
				OrderList.class, id);
